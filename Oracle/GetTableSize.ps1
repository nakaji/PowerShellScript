...
Write-Host ("  {0, -10} {1, -30} {2, 10} {3, 15}" -F "TYPE", "OBJECT_NAME" ,"SIZE(MB)", "RECORD_COUNT")
while ( $reader.read() )
{
    if ($reader[1] -eq "TABLE"){
        ...
        Write-Host ("  {0, -10} {1, -30} {2, 10} {3, 15}" -F $reader[1] ,$reader[0] ,$reader[2], $countReader[0])
    }else{
        Write-Host ("  {0, -10} {1, -30} {2, 10}" -F $reader[1] ,$reader[0] ,$reader[2])
    }
